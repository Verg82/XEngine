...
void MeshRenderer::LoadShaders()
{
    ...
    const uint32 NumShaders = (2 * 2 * 2 * 2 * uint32(FixedFilterSize::NumValues) * uint32(ShadowMode::NumValues) * uint32(VSMFormat::NumValues))
                            + (uint32(ShadowMode::NumValues) * uint32(ShadowMSAA::NumValues) *  uint32(VSMFormat::NumValues))
                            + ((MaxBlurRadius + 1) * 2) + 2 + 2 + 3 + 1 + 2;
    uint32 currShader = 0;
    for(uint32 visualizeCascades = 0; visualizeCascades < 2; ++visualizeCascades)
    {
        for(uint32 usePlaneBias = 0; usePlaneBias < 2; ++usePlaneBias)
        {
            for(uint32 filterCascades = 0; filterCascades < 2; ++filterCascades)
            {
                for(uint32 randomizeOffsets = 0; randomizeOffsets < 2; ++randomizeOffsets)
                {
                    for(uint32 filterSize = 0; filterSize < uint32(FixedFilterSize::NumValues); ++filterSize)
                    {
                        for(uint32 shadowMode = 0; shadowMode < uint32(ShadowMode::NumValues); ++shadowMode)
                        {
                            for(uint32 evsmFormat = 0; evsmFormat < uint32(VSMFormat::NumValues); ++evsmFormat)
                            {
                                if(RenderShaderProgress(currShader++, NumShaders) == false)
                                    return;

                                if((filterSize > 0 && (shadowMode != uint32(ShadowMode::FixedSizePCF) && shadowMode != uint32(ShadowMode::OptimizedPCF)))
                                    || (randomizeOffsets && shadowMode != uint32(ShadowMode::RandomDiscPCF))
                                    || (usePlaneBias && AppSettings::UseVSM(shadowMode)))
                                {
                                    continue;
                                }

                                PixelShaderPtr& shader = meshPS[visualizeCascades][usePlaneBias][filterCascades]
                                                               [randomizeOffsets][filterSize][shadowMode][evsmFormat];

                                CompileOptions opts;
                                opts.Add("VisualizeCascades_", visualizeCascades);
                                opts.Add("UsePlaneDepthBias_", usePlaneBias);
                                opts.Add("FilterAcrossCascades_", filterCascades);
                                opts.Add("FilterSize_", AppSettings::FixedFilterKernelSize(filterSize));
                                opts.Add("ShadowMode_", shadowMode);
                                opts.Add("VSMFormat_", evsmFormat);
                                opts.Add("RandomizeOffsets_", randomizeOffsets);
                                shader = CompilePSFromFile(device, L"Mesh.hlsl", "PS", "ps_5_0", opts);
                            }
                        }
                    }
                }
    ...
    for(uint32 shadowMode = 0; shadowMode < uint32(ShadowMode::NumValues); ++shadowMode)
    {
        for(uint32 msaaMode = 0; msaaMode < uint32(ShadowMSAA::NumValues); ++msaaMode)
        {
            for(uint32 evsmFormat = 0; evsmFormat < uint32(VSMFormat::NumValues); ++evsmFormat)
            {
                ...
                PixelShaderPtr& shader = vsmConvertPS[shadowMode][msaaMode][evsmFormat];
...
void MeshRenderer::CreateShadowMaps()
{
    ...
    if(AppSettings::UseVSM())
    {
        ...
        {
            if(AppSettings::VSMFormat == VSMFormat::VSM16Bit)
...
void MeshRenderer::Update()
{
    if(AppSettings::ShadowMapSize.Changed() || AppSettings::ShadowMode.Changed()
        || AppSettings::ShadowMSAA.Changed() || AppSettings::VSMFormat.Changed()
                                ...
                                Float3 cascadeScale, Float3 cascade0Scale)
{
    ...
    ID3D11PixelShader* ps = vsmConvertPS[AppSettings::ShadowMode][AppSettings::ShadowMSAA][AppSettings::VSMFormat];
                          ...
                          const Float4x4& characterWorld)
{
    ...
    uint32 usePlaneBias = AppSettings::UseVSM() ? 0 : AppSettings::UsePlaneDepthBias;
    ID3D11PixelShader* ps = meshPS[AppSettings::VisualizeCascades][usePlaneBias][AppSettings::FilterAcrossCascades]
                                  [randomizeOffsets][filterSize][AppSettings::ShadowMode][AppSettings::VSMFormat];
    context->PSSetShader(ps, nullptr, 0);
