...
float4 BlurVSM(in VSOutput input) : SV_Target0
{
    #if Vertical_
        float2 evsmMapSize;
        VSMMap.GetDimensions(evsmMapSize.x, evsmMapSize.y);
    #else
        float2 evsmMapSize;
        float numSlices;
        VSMMap.GetDimensions(evsmMapSize.x, evsmMapSize.y, numSlices);
    #endif
    ...
    const float Radius = KernelSize / 2.0f;
        ...
        if(KernelSize > 1.0f)
        {
            ...
            {
                float4 sample = BlurSample(input.Position.xy, i, evsmMapSize);
        ...
        {
            return BlurSample(input.Position.xy, 0, evsmMapSize);
        }
    #else
        float4 sum = 0.0f;
