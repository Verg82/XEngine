                         ...
                         in float3 shadowPosDY, uint cascadeIdx)
{
    ...
    return ChebyshevUpperBound(occluder, depth, 0.0001f, LightBleedingReduction);
}
...
float sampleShadowMap(float2 base_uv, float u, float v, float2 shadowMapSizeInv, uint cascadeIdx, float depth, float2 receiverPlaneDepthBias) {
