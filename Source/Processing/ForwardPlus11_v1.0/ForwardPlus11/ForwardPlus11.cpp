...
// Rasterizer states
ID3D11RasterizerState*      g_pDisableCulling = NULL;
...
// UI control IDs
//--------------------------------------------------------------------------------------
#define IDC_BUTTON_TOGGLEFULLSCREEN          1
#define IDC_BUTTON_TOGGLEREF                 2
#define IDC_BUTTON_CHANGEDEVICE              3
#define IDC_CHECKBOX_ENABLE_LIGHT_DRAWING    4
#define IDC_STATIC_NUM_POINT_LIGHTS          5
#define IDC_SLIDER_NUM_POINT_LIGHTS          6
#define IDC_STATIC_NUM_SPOT_LIGHTS           7
#define IDC_SLIDER_NUM_SPOT_LIGHTS           8
#define IDC_CHECKBOX_ENABLE_LIGHT_CULLING    9
#define IDC_CHECKBOX_ENABLE_DEPTH_CULLING    10
#define IDC_CHECKBOX_ENABLE_DEBUG_DRAWING    12
#define IDC_RADIOBUTTON_DEBUG_DRAWING_ONE    13
#define IDC_RADIOBUTTON_DEBUG_DRAWING_TWO    14
#define IDC_TILE_DRAWING_GROUP               0x0100
...
int WINAPI wWinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow )
{
    ...
    DXUTCreateWindow( L"ForwardPlus11 v1.0" );
...
void InitApp()
{
    ...
    g_HUD.m_GUI.AddCheckBox( IDC_CHECKBOX_ENABLE_DEPTH_CULLING, L"Enable Depth Culling", AMD::HUD::iElementOffset, iY += AMD::HUD::iElementDelta, AMD::HUD::iElementWidth, AMD::HUD::iElementHeight, true );
...
void RenderText()
{
    ...
    if( bDebugDrawingEnabled )
    {
        ...
        g_Util.RenderLegend( g_pTxtHelper, TEXT_LINE_HEIGHT, D3DXCOLOR( 1.0f, 1.0f, 1.0f, 0.75f ), !bDebugDrawMethodOne );
    }
}
                                     ...
                                     void* pUserContext )
{
    ...
    D3DXVECTOR3 vSceneMin, vSceneMax;
    ...
    g_Util.CalculateSceneMinMax( g_SceneMesh, &vSceneMin, &vSceneMax );
    ...
    V_RETURN( pd3dDevice->CreateRasterizerState( &RasterizerDesc, &g_pDisableCulling ) );
    ...
    if( bFirstPass )
    {
        ...
        D3DXVECTOR3 vSceneCenter  = 0.5f * (vSceneMax + vSceneMin);
        D3DXVECTOR3 vSceneExtents = 0.5f * (vSceneMax - vSceneMin);
        D3DXVECTOR3 vBoundaryMin  = vSceneCenter - 2.0f*vSceneExtents;
        D3DXVECTOR3 vBoundaryMax = vSceneCenter + 2.0f*vSceneExtents;
        D3DXVECTOR3 vBoundaryDiff = 4.0f*vSceneExtents;  // vBoundaryMax - vBoundaryMin
        g_fMaxDistance = D3DXVec3Length(&vBoundaryDiff);
        D3DXVECTOR3 vEye = vSceneCenter - D3DXVECTOR3(0.67f*vSceneExtents.x, 0.5f*vSceneExtents.y, 0.0f);
        D3DXVECTOR3 vAt = vSceneCenter - D3DXVECTOR3(0.0f, 0.5f*vSceneExtents.y, 0.0f);
        ...
        ForwardPlusUtil::InitLights( vSceneMin, vSceneMax );
    }
                                         ...
                                         const DXGI_SURFACE_DESC* pBackBufferSurfaceDesc, void* pUserContext )
{
    ...
    g_Camera.SetProjParams( D3DX_PI / 4, fAspectRatio, g_fMaxDistance, 0.1f );
                                 ...
                                 float fElapsedTime, void* pUserContext )
{
    ...
    bool bCullingEnabled = g_HUD.m_GUI.GetCheckBox( IDC_CHECKBOX_ENABLE_LIGHT_CULLING )->GetEnabled() &&
            g_HUD.m_GUI.GetCheckBox( IDC_CHECKBOX_ENABLE_LIGHT_CULLING )->GetChecked();
    pScenePS = bCullingEnabled ? pScenePS : g_pScenePSNoCull;
    pScenePSAlphaTest = bCullingEnabled ? pScenePSAlphaTest : g_pScenePSNoCullAlphaTest;
    ...
    bool bDepthCullingEnabled = g_HUD.m_GUI.GetCheckBox( IDC_CHECKBOX_ENABLE_DEPTH_CULLING )->GetEnabled() &&
            g_HUD.m_GUI.GetCheckBox( IDC_CHECKBOX_ENABLE_DEPTH_CULLING )->GetChecked();
    pLightCullCS = bDepthCullingEnabled ? pLightCullCS : g_pLightCullCSNoDepth;
    pDepthSRV = bDepthCullingEnabled ? pDepthSRV : NULL;
    ...
    // compute shader
    D3DXMATRIX mInvProj;
    D3DXMatrixIdentity(&mInvProj);
    mInvProj._11 = 1.0f / mProj._11;
    mInvProj._22 = 1.0f / mProj._22;
    mInvProj._33 = 0.0f;
    mInvProj._34 = 1.0f / mProj._43;
    mInvProj._43 = 1.0f;
    mInvProj._44 = -mProj._33 / mProj._43;
    ...
    D3DXMatrixTranspose( &pPerFrame->m_mProjection, &mProj );
    D3DXMatrixTranspose( &pPerFrame->m_mProjectionInv, &mInvProj );
    pPerFrame->m_vCameraPos = *(g_Camera.GetEyePt());
    pPerFrame->m_fAlphaTest = bMSAAEnabled ? 0.003f : 0.5f;
    ...
    D3DXMatrixTranspose( &pPerObject->m_mWorldViewProjection, &mWorldViewProjection );
    D3DXMatrixTranspose( &pPerObject->m_mWorldView, &mWorldView );
    D3DXMatrixTranspose( &pPerObject->m_mWorld, &mWorld );
    pPerObject->m_MaterialAmbientColorUp = D3DXVECTOR4( 0.013f, 0.015f, 0.050f, 1.0f );
    pPerObject->m_MaterialAmbientColorDown = D3DXVECTOR4( 0.0013f, 0.0015f, 0.0050f, 1.0f );
    ...
    if( g_ShaderCache.ShadersReady() )
    {
        ...
        TIMER_Begin( 0, L"Depth pre-pass" );
        {
            ...
            pd3dImmediateContext->RSSetState( g_pDisableCulling );
    ...
    else
    {
        ...
        pd3dImmediateContext->OMSetRenderTargets( 1, (ID3D11RenderTargetView *const *)&pRTV, g_pDepthStencilView );
        g_ShaderCache.RenderProgress( g_pTxtHelper, TEXT_LINE_HEIGHT, D3DXCOLOR( 1.0f, 1.0f, 0.0f, 1.0f ) );
    }
...
void CALLBACK OnD3D11DestroyDevice( void* pUserContext )
{
    ...
    SAFE_RELEASE( g_pDisableCulling );
    ...
    if( bKeyDown )
    {
        switch( nChar )
        {
            case VK_F1:
                ...
                break;
            case VK_F4:
                if( !bAltDown ) DXUTSnapD3D11Screenshot( L"screenshot.bmp", D3DX11_IFF_BMP );
                break;
        }
    }
}
        ...
        case IDC_CHECKBOX_ENABLE_LIGHT_CULLING:
            {
                bool bCullingEnabled = g_HUD.m_GUI.GetCheckBox( IDC_CHECKBOX_ENABLE_LIGHT_CULLING )->GetEnabled() &&
                    g_HUD.m_GUI.GetCheckBox( IDC_CHECKBOX_ENABLE_LIGHT_CULLING )->GetChecked();
                g_HUD.m_GUI.GetCheckBox( IDC_CHECKBOX_ENABLE_DEBUG_DRAWING )->SetEnabled(bCullingEnabled);
                g_HUD.m_GUI.GetCheckBox( IDC_CHECKBOX_ENABLE_DEPTH_CULLING )->SetEnabled(bCullingEnabled);
                if( bCullingEnabled == false )
                {
...
HRESULT AddShadersToCache()
{
    ...
    AMD::ShaderCache::Macro ShaderMacroUseDepthCulling;
    wcscpy_s( ShaderMacroUseDepthCulling.m_wsName, AMD::ShaderCache::m_uMACRO_MAX_LENGTH, L"USE_DEPTH_CULLING" );
