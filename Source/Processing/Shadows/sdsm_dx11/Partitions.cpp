...
float ComputeMaxDistanceToAABBCorner(const D3DXVECTOR3& p,
                                     ...
                                     const D3DXVECTOR3& aabbExtents)
{
    // Compute quadrant of p with respect to AABB center
    // Make any zero's positive (doesn't matter which we choose)
    D3DXVECTOR3 quadrant;
    quadrant.x = p.x > aabbCenter.x ? 1.0f : -1.0f;
    quadrant.y = p.y > aabbCenter.y ? 1.0f : -1.0f;
    quadrant.z = p.z > aabbCenter.z ? 1.0f : -1.0f;

    // Now the furthest corner is the opposite quadrant
    D3DXVECTOR3 furthest = aabbCenter;
    furthest.x -= quadrant.x * aabbExtents.x;
    furthest.y -= quadrant.y * aabbExtents.y;
    furthest.z -= quadrant.z * aabbExtents.z;

    D3DXVECTOR3 v = p - furthest;
